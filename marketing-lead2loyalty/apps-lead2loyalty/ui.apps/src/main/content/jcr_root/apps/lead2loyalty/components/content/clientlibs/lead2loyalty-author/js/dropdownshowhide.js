/**
 * Extension to the standard dropdown/select component. It enables hiding/unhiding of other components based on the
 * selection made in the dropdown/select.
 *
 *
 * How to use:
 *
 * 1. Add the class cq-dialog-dropdown-showhide to the dropdown/select element
 *			Example: granite:class="cq-dialog-dropdown-showhide"
 * 2. Add the data attribute cq-dialog-dropdown-showhide-target to the dropdown/select element, value should be the selector,
 * 	  usually a specific class name, to find all possible target elements that can be shown/hidden
 *			Example: cq-dialog-dropdown-showhide-target=".showhide-target"
 * //Can add cq:hideOnEdit=(Boolean)false to each of the items under the dropdown/select element
 * 3. Add the target class to each target component that can be shown/hidden
 *			Example: granite:class="showhide-target"
 * 4. Add the class hide to each target component to make them initially hidden
 *			Example: granite:class="hide showhide-target"
 * 5. Add the data attribute showhidetargetvalue to each target component, the value should equal the value of the select
 *    option that will unhide this element.
 *			Example: showhidetargetvalue="other"
 */
(function (document, $) {
    "use strict";
